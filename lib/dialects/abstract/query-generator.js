        ...
        var generateJoinQueries = function(include, parentTable) {
          ...
          if (options.includeIgnoreAttributes !== false) {
           ...
           //attr = 'ST_AsGeoJSON('+self.quoteIdentifiers(attr)+') AS '+ self.quoteIdentifiers(attr);
              ...
              var prefix;
              if (verbatim === true) {
                prefix = include.model._isGeometryAttribute(attr) ? 'ST_AsGeoJSON(' + attr + ')' : prefix = attr;
              } else {
                prefix = include.model._isGeometryAttribute(attr) ? 'ST_ASGeoJSON(' + self.quoteIdentifier(as) + '.' + self.quoteIdentifier(attr) + ')': self.quoteIdentifier(as) + '.' + self.quoteIdentifier(attr);
              }
