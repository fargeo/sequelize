        ...
        var generateJoinQueries = function(include, parentTable) {
          ...
          if (options.includeIgnoreAttributes !== false) {
            attributes = include.attributes.map(function(attr) {
              ...
              isGeom = include.model._isGeometryAttribute(attr);

              if (verbatim === true) {
                prefix = isGeom ? 'ST_AsGeoJSON(' + attr + ')' : attr;
              } else {
                prefix = isGeom ? 'ST_ASGeoJSON(' + self.quoteIdentifier(as) + '.' + self.quoteIdentifier(attr) + ')': self.quoteIdentifier(as) + '.' + self.quoteIdentifier(attr);
              }
